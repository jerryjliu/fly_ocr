...
import {renderSocial,type SocialData} from '../lib/social-layout';
import {socialChapters,socialState} from '../lib/social-timeline';
...
async function init(){
  ...
  for(const chapter of socialChapters){
  ...
  function frame(chapterIndex:number,seconds:number){
    const d=data[chapterIndex],chapter=socialChapters[chapterIndex],{done,position,finished}=socialState(seconds,d.run.events.length,chapter.seconds);
    ...
    const global=socialChapters.slice(0,chapterIndex).reduce((a,ch)=>a+ch.seconds,0)+seconds;
    ...
    renderSocial(c,d,chapterIndex,seconds,global,scene);
    return {done,event_id:e.id,character:e.character,finished};
  }
  Object.assign(window,{social:{canvas,frame,chapters:socialChapters,ready:true}});
  frame(0,0);
}
init().catch(error=>{console.error(error);Object.assign(window,{socialError:String(error)});});
